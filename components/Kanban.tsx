...
const Kanban: React.FC<KanbanProps> = ({ tasks, onMoveTask, onAddTask, onUpdateTask, sectors, team, currentUser }) => {
  ...
  const [newTaskLead, setNewTaskLead] = useState(currentUser.id);
  const [newTaskSector, setNewTaskSector] = useState(sectors[0]?.id || '');
  ...
  const handleCreateTask = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTaskTitle || !newTaskSector) return;
    ...
    <div className="flex-1 flex flex-col h-full bg-white overflow-hidden">
      ...
      <div className="flex-1 overflow-x-auto p-8 custom-scrollbar">
        <div className="flex gap-6 h-full items-start">
          {columns.map((col) => (
            <div key={col} className="w-[320px] flex-shrink-0 bg-kanban-bg rounded-xl p-3 flex flex-col max-h-full">
              <h3 className="font-bold text-xs uppercase tracking-wider text-slate-500 mb-4 px-2">{col}</h3>
              <div className="flex flex-col gap-4 overflow-y-auto custom-scrollbar min-h-[100px]">
                {tasks.filter(t => t.status === col).map(task => {
                  ...
                  return (
                    <div key={task.id} onClick={() => setSelectedTask(task)} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:border-primary/50 transition-all group cursor-pointer">
          ...
          <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden max-h-[90vh]">
            ...
            <div className="w-72 bg-slate-50 border-l p-8 flex flex-col gap-6">
              ...
              <div className="mt-auto">
                <button className="w-full py-2.5 bg-slate-900 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-lg">Finalizar Projeto</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={newTaskLead} onChange={e => setNewTaskLead(e.target.value)} className="w-full border-slate-200 bg-slate-50 rounded-xl py-2 px-4 text-sm">
                    {team.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Prazo de Entrega (Selecione no Calendário)</label>
              ...
              <div className="border-t pt-4">
                ...
                <div className="flex flex-col gap-3">
                  {newSubTasks.map((st, i) => (
                    <div key={i} className="flex gap-2 items-center">
                      <select value={st.userId} onChange={e => {
                        ...
                        setNewSubTasks(copy);
                      }} className="w-32 text-xs border-slate-100 bg-slate-50 rounded-lg">
                        {team.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                      </select>
                  ...
                  <button onClick={() => setNewSubTasks([...newSubTasks, {userId: team[0].id, desc: ''}])} className="text-[10px] font-bold text-primary flex items-center gap-1 hover:underline">
